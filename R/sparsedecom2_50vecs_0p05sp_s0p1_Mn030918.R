...
#reuses fused eigenregions for best performing fold Alex Badea 24 august 2018
...
#mypath<-'/Users/alex/Documents/GitHub/adforesight/'
setwd(mypath)
source(paste(mypath, '/R/myr2score.R',sep=''))

mysp <- 0.05 #0.05  # 0.01  # 0.05 #0.2 #0.05 # was 0.005 sparseness
mynvecs <- 50 # 50 #put back to 50 alex #tested with 10
myell1 <- 1 # make this smaller 0.5, Brian says it is not what i think just switch between l0 and l1
myits<-5 #5 #put back to 5
mysmooth<-0.01 #0  # was 0.01
myclus<-150 #was 250

#build a place to save results
extension<-paste('Mn', 'sp', toString(mysp), 'vecs', toString(mynvecs), 's', toString(mysmooth),'clus', toString(myclus), sep='') # 'JACsp0p005s0'
...
for (fold in 1:k){
 ...
 # eanat_mang<-sparseDecom( inmatrix=mang.train, inmask=mask, nvecs=mynvecs, sparseness=mysp, cthresh=myclus, its=myits, mycoption=0,verbose=1,statdir=output.path, smooth=mysmooth )
  myeig2<-sparseDecom2(inmatrix = list(mang.train,as.matrix(behav.train$d4)),its = 2, mycoption = 0, sparseness = c(0.02,1), nvecs = 5, verbose=1, statdir=paste(output.path2))
  ...
  colnames(projs.train) <- c('Dist_4', paste0('Proj', c(1:ncol( imgmat_train_mang))))
  ...
  ggtitle(paste("RMSE=",formatC(RMSE,digits=2, format="f"), "p=",formatC(myps[fold],digits=2, format="f"), " BIC=", formatC(BIC(mymodel),digits=2, format="f")))
  ...
  #alex to figure out mask format
  jeanat_mang <- joinEigenanatomy(mang.train, mask=NA, eanat_mang$eigenanatomyimages, graphdensity=c(0.1,0.2, 0.4,0.6), joinMethod='multilevel', verbose=TRUE)
  ...
  myplot + theme(panel.grid.minor = element_blank(), panel.grid.major = element_blank(),
                 ...
                 axis.line.x = element_line(colour = 'black', size=0.5, linetype='solid'),
                 axis.line.y = element_line(colour = 'black', size=0.5, linetype='solid'))+
                 ggtitle(paste("RMSE=",formatC(RMSE2,digits=2,format="f"), "R2score=",formatC(myR2score[fold],digits=2, format="f"), "p=",formatC(myps[fold],digits=2, format="f"),"BIC=", formatC(myBICs[fold],digits=2, format="f")))
  ...
  save(mymodel, file=paste(output.path , "behavmodel", toString(fold), ".Rdata", sep=''))
...
write.csv(mycorsdf_eig2d4, file = paste(output.path ,extension,'eig2d4corsvalid.csv',sep=''))

myperf<-data.frame(rbind(distpred,dist4.valid),row.names=c("d_predicted","d_valid"))
write.csv(myperf, file = paste(output.path ,extension,'distances4_valid.csv',sep=''))
