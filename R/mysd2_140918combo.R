...
mysp <- 0.5 #0.05  # 0.01  # 0.05 #0.2 #0.05 # was 0.005 sparseness
...
myits<-15 #15 #5 #put back to 5
mysmooth<-0.1 # 0.1 #0.01 #0  # was 0.01
myclus<-250 #was 250

#build a place to save results
extension<-paste('pCOMBO', 'sp', toString(mysp), 'vecs', toString(mynvecs), 's', toString(mysmooth),'clus', toString(myclus), sep='') # 'JACsp0p005s0'
...
for (myfold in 1:k){
 # for (myfold in 3){
  ...
  # rt<-as.integer(unlist(res_test[-myfold]))
  # rows.train<-myindex[!(myindex %in% res_test[myfold])]
  # rows.test<-res_test[myfold]
  # 
  # training_set <- train_set[!(rows.train %in% rows.train[myfold]), ] ### Create training set for this iteration
  # testing_set <- train_set[myfold, ] ### training_set %>% kable() #showoffish
  # testing_set %>% kable() #showoffish
  
  # rows.train<-as.integer(res_train[[myfold]])
  # rows.test<-as.integer(res_test[[myfold]])
  ...
  start_time <- Sys.time()
 #negative sparseness is what?
  myeig2_mang<-sparseDecom2(inmatrix = list(mang.train,as.matrix(behav.train$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs = mynvecs, verbose=1, statdir=paste(output.path2))
  myeig2_jac<-sparseDecom2(inmatrix = list(jac.train,as.matrix(behav.train$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs = mynvecs, verbose=1, statdir=paste(output.path2))
  myeig2_chi<-sparseDecom2(inmatrix = list(chi.train,as.matrix(behav.train$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs =mynvecs, verbose=1, statdir=paste(output.path2))
  ...
  ####start do combo alex
  ncolcombo<-ncol( imgpredtrain_jac)+ncol( imgpredtrain_mang)+ncol( imgpredtrain_chi)
  
  projs.train <- data.frame(cbind(dist4.train, imgpredtrain_jac , imgpredtrain_mang ,imgpredtrain_chi)) # column combind the behavior wth the projections
  colnames(projs.train) <- c('Dist_4', paste0('Proj', c(1:ncolcombo)))
  projs.test <- data.frame(cbind(dist4.test, imgpredtest_jac,  imgpredtest_mang , imgpredtest_chi)) # column combind the behavior wth the projections
...
projs.valid <- data.frame(cbind(dist4.valid,imgmat_jac_valid,imgmat_mang_valid,imgmat_chi_valid))
...
myeig2_mang_valid<-sparseDecom2(inmatrix = list(mang.valid,as.matrix(behavior$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs = mynvecs, verbose=1, statdir=paste(output.path2))
myeig2_jac_valid<-sparseDecom2(inmatrix = list(jac.valid,as.matrix(behavior$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs = mynvecs, verbose=1, statdir=paste(output.path2))
myeig2_chi_valid<-sparseDecom2(inmatrix = list(chi.valid,as.matrix(behavior$d4)),its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c(mysp,1), nvecs =mynvecs, verbose=1, statdir=paste(output.path2))

e2i_mang_valid<-matrixToImages(t((myeig2_mang_valid$eig1)),mask = mask)
e2i_jac_valid<-matrixToImages(t((myeig2_jac_valid$eig1)),mask = mask)
e2i_chi_valid<-matrixToImages(t((myeig2_chi_valid$eig1)),mask = mask)


for (i in 1:mynvecs){
  antsImageWrite(e2i_mang_valid[[i]],paste(output.path,extension,'full_eig' ,as.character(i), '_Mn.nii.gz',sep=''))
  antsImageWrite(e2i_jac_valid[[i]],paste(output.path,extension,'full_eig' ,as.character(i), '_jac.nii.gz',sep=''))
  antsImageWrite(e2i_chi_valid[[i]],paste(output.path,extension,'full_eig' ,as.character(i), '_chi.nii.gz',sep=''))
}
