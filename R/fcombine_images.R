...
output.path <-'/Users/omega/Documents/Natalie/Thesis/eigseg_data/sparsedecom2_region_breakdown/sparse01_smooth01/projall_noscaling/'
...
for ( j in 1:length(label.ind)){
  ...
  eanat_region_jac<-sparseDecom2(inmatrix = list(jac.train,behav.matrix), inmask = c(mask, NA), sparseness = c(0.01,1),
  ...
  eanat_region_mang<-sparseDecom2(inmatrix = list(mang.train,behav.matrix), inmask = c(mask, NA), sparseness = c(0.01,1),
                                  nvecs = 1, its = 10, cthresh = c(100, 0), mycoption = 0, smooth = 0.01,
                                  initializationList = ccainit$initlist,priorWeight = 0.5)
  eanat_region_suscept<-sparseDecom2(inmatrix = list(suscept.train,behav.matrix), inmask = c(mask, NA), sparseness =    c(0.01,1),                  nvecs = 1, its = 10, cthresh = c(100, 0), mycoption = 0, smooth = 0.01,
                                     initializationList = ccainit$initlist,priorWeight = 0.5)
