...
label.ind <- c(119, 20, 19,42,47,51,54,57,59,64,65,120, 121,122, 123) #20,62,63,73,81,83,91,91,119; c(62,122, 67) #

#Both (Left and Right) ROIS
#for ( j in 2:length(label.ind)){
  for ( j in 1:1){
    #for ( j in 7:length(label.ind)){
...
#####start alex folds
  ...
  #by="genotype", can be used to color point but i need to put it in a data frame
  myplot<- visreg(mymodel, gg=TRUE, overlay=TRUE, scale='linear', plot=TRUE, xlim=c(0,max(dist4.valid)),ylim=c(0,max(dist4.valid))) 
