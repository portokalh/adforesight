...
extension<-paste('SINGLEJAC', 'sp', toString(mysp), 'vecs', toString(mynvecs), 's', toString(mysmooth),'clus', toString(myclus), sep='') # 'JACsp0p005s0'
...
for (myfold in 1:k){
 ...
 #negative sparseness is what?
  ...
  imgpredtrain_mang<-mang.train %*% t(myeig2_mang$eigenanatomyimages)
  imgpredtest_mang<-mang.test %*% t(myeig2_mang$eigenanatomyimages)
  ...
  e2i_mang<-matrixToImages(((myeig2_mang$eigenanatomyimages)),mask = mask)
