...
label.ind <- c(120, 119, 122, 19,42,47,51,54,57,59,64,65,120, 121, 122, 123) #20,62,63,73,81,83,91,91,119
...
#for ( j in 2:length(label.ind)){
 # for ( j in 1:length(label.ind)){
  ...
  output.path <- paste(output.path0, '/', mylabel, '/', sep='') #sd2_projall_noscale/'
  ...
  antsImageWrite(mask2,paste(output.path,'ccainit_mask' ,as.character(labeled.set$Abbreviation[label.ind[j]]), '.nii.gz',sep=''))
  ...
  #overwrite mats with smaller mask restricted to label
  Mn_mat <-imagesToMatrix(Mn_files,mask2)
  jac_mat <-imagesToMatrix(jac_files,mask2)
  chi_mat <-imagesToMatrix(chi_files,mask2)
...
#####start alex folds
  for (myfold in 1:k){
    ...
    # myeig2_Mn<-sparseDecom2(inmatrix = list(Mn.train,as.matrix(behav.train$d4)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = ccainit$initlist, priorWeight = 0.9)
    # myeig2_chi<-sparseDecom2(inmatrix = list(chi.train,as.matrix(behav.train$d4)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = ccainit$initlist, priorWeight = 0.9)
    # 
    myeig2_jac<-sparseDecom2(inmatrix = list(jac.train,as.matrix(behav.train$d4)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
    myeig2_Mn<-sparseDecom2(inmatrix = list(Mn.train,as.matrix(behav.train$d4)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
    myeig2_chi<-sparseDecom2(inmatrix = list(chi.train,as.matrix(behav.train$d4)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
  ...
  behavior <- read.csv('./mydata/All_Behavior.csv')
  behavior<-behavior[c(1:24),];
  # files_jac <- list.files(path = paste(output.path,sep=''), pattern=paste('*', 'fold', toString(myfold), '_jac.nii.gz', sep=''),full.names = T,recursive = T)
  # jac_mat <- imagesToMatrix(files_jac,mask2)
  # files_Mn <- list.files(path = paste(output.path,sep=''), pattern=paste('*', 'fold', toString(myfold), '_Mn.nii.gz', sep=''),full.names = T,recursive = T)
  # Mn_mat <- imagesToMatrix(files_Mn,mask2)
  # files_chi <- list.files(path = paste(output.path,sep=''), pattern=paste('*', 'fold', toString(myfold), '_chi.nii.gz', sep=''),full.names = T,recursive = T)
  # chi_mat <- imagesToMatrix(files_chi,mask2)
  # 
  # Mn_files <- list.files(path = "./mydata/imdata/", pattern = "T2_to_MDT",full.names = T,recursive = T)
  jac_files <- list.files(path = "./mydata/imdata/", pattern = "jac_to_MDT",full.names = T,recursive = T)
  chi_files <- list.files(path = "./mydata/imdata/", pattern = "X_to_MDT",full.names = T,recursive = T)
  ...
  myeig2_jac<-sparseDecom2(inmatrix = list(jac.valid,as.matrix(dist4.valid)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
  myeig2_Mn<-sparseDecom2(inmatrix = list(Mn.valid,as.matrix(dist4.valid)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
  myeig2_chi<-sparseDecom2(inmatrix = list(chi.valid,as.matrix(dist4.valid)),inmask=list(mask2 ,NA), its = myits, cthresh=c(myclus,0), smooth = mysmooth, mycoption = 0, sparseness = c( mysp, -1 ), nvecs = mynvecs, verbose=1, statdir=paste(output.path),initializationList = list(mask2), priorWeight = 0.9)
  ...
  imgmat_Mn_valid <- Mn.valid %*% (myeig2_Mn$eig1) # [24,numvox] [nvecsx3,numvox]
  imgmat_jac_valid <- jac.valid %*% (myeig2_jac$eig1)
  imgmat_chi_valid <- chi.valid %*% (myeig2_chi$eig1)
