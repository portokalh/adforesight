...
extension<-paste(mysuffix,'singleROI_pred',sep='')
...
#####start alex folds
  for (myfold in 1:k){
    ...
    ####start do combo alex
    ncolcombo<-ncol( imgpredtrain_jac)
    projs.train <- data.frame(cbind(dist4.train, imgpredtrain_jac )) # column combind the behavior wth the projections
    colnames(projs.train) <- c('Dist_4', paste0('Proj', c(1:ncolcombo)))
    projs.test <- data.frame(cbind(dist4.test, imgpredtest_jac)) # column combind the behavior wth the projections
  ...
  projs.valid <- data.frame(cbind(dist4.valid,imgmat_jac_valid))
  ...
  rd4 <- t(t(dist4.valid)[rep(1,c(mynvecs)),])
  pcor<-c(numcols)
  corval<-c(numcols)
